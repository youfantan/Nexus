...
Socket::Socket(SockType typ) : type_(typ) {
    if (typ == SockType::IPPROTO_IPV4) fd_ = socket(AF_INET, SOCK_STREAM, 0);
    else if (typ == SockType::IPPROTO_IPV6) fd_ = socket(AF_INET6, SOCK_STREAM, 0);
    else {
        invalid_ = true;
        fd_ = 0;
    }
...
bool Socket::bind(const std::string &addr, uint16_t port) {
    ...
    if (na.type() != SockType::INVALID) {
        if (na.type() == SockType::IPPROTO_IPV4 && type_ != SockType::IPPROTO_IPV4) {
            invalid_ = true;
            return false;
        } else if (na.type() == SockType::IPPROTO_IPV6 && type_ != SockType::IPPROTO_IPV6) {
            invalid_ = true;
            return false;
        }
...
bool Socket::bind(sockaddr_in6 addrv6, uint16_t port) {
    if (type_ != SockType::IPPROTO_IPV6) {
        invalid_ = true;
        return false;
    }
...
bool Socket::bind(sockaddr_in addrv4, uint16_t port) {
    if (type_ != SockType::IPPROTO_IPV4) {
        invalid_ = true;
        return false;
    }
...
Socket Socket::accept() {
    ...
    if (sock != INVALID_SOCKET) {
        if (ss.ss_family == AF_INET) return {sock, SockType::IPPROTO_IPV4};
        else if (ss.ss_family == AF_INET6) return {sock, SockType::IPPROTO_IPV6};
        else if (ss.ss_family == AF_UNIX) return {sock, SockType::IPPROTO_UNIX};
