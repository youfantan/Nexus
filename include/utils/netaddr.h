    ...
    public:
        explicit NetAddr(const std::string& addr, uint16_t port) : port_(htons(port)) {
            ...
            if (inet_pton(AF_INET, addr.c_str(), &v4adr) == 1) {
                ...
                if (v4.is_valid()) {
                    memcpy(inaddr_, reinterpret_cast<char*>(&v4.result()), sizeof(v4.result()));
                } else {
                    //try IPV6 then
                    type_ = SockType::IPPROTO_IPV6;
                    auto v6 = DNSLookUpV6(addr);
                    if (v6.is_valid()) {
                        memcpy(inaddr_, reinterpret_cast<char*>(&v6.result()), sizeof(v6.result()));
                    } else {
                        //invalid_ address
        ...
        UniqueFlexHolder<sockaddr> addr() {
            ...
            if (type_ == SockType::IPPROTO_IPV4) {
                ...
                holder.assign(&sa);
            } else if (type_ == SockType::IPPROTO_IPV6) {
                sockaddr_in6 sa{};
        ...
        int size() {
            if (type_ == SockType::IPPROTO_IPV4) return sizeof(sockaddr_in);
            else if (type_ == SockType::IPPROTO_IPV6) return sizeof(sockaddr_in6);
            else return 0;
        }
    };
}