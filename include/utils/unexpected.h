...
#define BREAKPOINTM(msg) breakpoint(__FILE__, __LINE__, msg)
    inline static constexpr std::string find_relative_path(const std::string& path) {
        std::string current_path = __FILE__;
        return path.substr(current_path.size() - 33);
    }
    inline static void breakpoint(const std::string& path, int line, const std::string& msg) {
        if (msg.empty()) {
